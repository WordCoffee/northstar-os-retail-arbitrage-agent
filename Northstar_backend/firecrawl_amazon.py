"""Firecrawl — Amazon product page adapter (free-tier fallback).

One request per ASIN via Firecrawl Scrape API (raw HTML). Parses the same
seller data contract via the shared amazon_seller_extract parser so
records are byte-compatible with other adapters.

Free tier (verified 2026-09-13): 1,000 pages/month, no card, 2 concurrent
requests, low rate limits.

Gating (fail-closed):
  FIRECRAWL_AMAZON_DETAIL_ENABLED=1 required for live calls. Default 0.
...
Discipline (mirrors firecrawl_costco.py / bright_data_costco.py):
  ...
  - sequential pacing: FIRECRAWL_AMAZON_REQUEST_DELAY_SECONDS (default 2.0)
  ...
  - exact HTTP status via LAST_HTTP_STATUS, typed LAST_ERROR string on every
    ...
    the run-directory convention, keyed with the `firecrawl_` prefix.

Env vars:
  FIRECRAWL_AMAZON_DETAIL_ENABLED        (default "0"; "1" enables live calls)
  FIRECRAWL_API_KEY                      (required for live; value never logged)
  FIRECRAWL_AMAZON_REQUEST_URL           (default https://api.firecrawl.dev/v2/scrape)
  FIRECRAWL_AMAZON_REQUEST_DELAY_SECONDS (default 2.0)
  FIRECRAWL_AMAZON_REQUEST_TIMEOUT_SECONDS (default 90)
"""
...
GATE_ENV = "FIRECRAWL_AMAZON_DETAIL_ENABLED"
DELAY_ENV = "FIRECRAWL_AMAZON_REQUEST_DELAY_SECONDS"
DEFAULT_DELAY_SECONDS = 2.0
KEY_ENV = "FIRECRAWL_API_KEY"
REQUEST_URL_ENV = "FIRECRAWL_AMAZON_REQUEST_URL"
DEFAULT_REQUEST_URL = "https://api.firecrawl.dev/v2/scrape"
TIMEOUT_ENV = "FIRECRAWL_AMAZON_REQUEST_TIMEOUT_SECONDS"
...
def _gate_enabled() -> bool:
    """Live-call gate: FIRECRAWL_AMAZON_DETAIL_ENABLED must be '1'."""
...
def _fetch_item_page(url: str) -> Optional[str]:
    ...
    for the Authorization header and never included in any persisted payload.
    """
...
def _persist_evidence(run_dir: str, ts: str, asin: str, raw_record: Dict, norm_record: Dict) -> List[str]:
    ...
    raw_path = os.path.join(raw_dir, f"firecrawl_{asin}_{ts}.json")
    norm_path = os.path.join(norm_dir, f"items_{asin}_firecrawl_{ts}.json")
...
def refresh_product_details(
    ...
    """Fetch Amazon product pages via Firecrawl.

    Gated (FIRECRAWL_AMAZON_DETAIL_ENABLED=1). One request per ASIN, zero
    ...
    if not _gate_enabled():
        msg = (
            f"[FirecrawlAmazon] BLOCKED: {GATE_ENV} != '1'. "
    ...
    run_dir = run_dir or os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "data", "enrich", "firecrawl-amazon", "runs"
    )
    ...
    for idx, asin in enumerate(asins or []):
        ...
        if html is None:
            ...
            fail_rec = {
                "run_id": "firecrawl_amazon_%s" % ts,
                "attempt": "firecrawl_amazon_%s" % asin,
        ...
        raw_record = {
            ...
            "platform": "firecrawl_scrape",
            "endpoint": _request_url(),
            "payload": {"url": url, "formats": ["rawHtml"], "maxAge": 0, "timeout": 60000, "proxy": "auto"},
...
def _cli(argv=None):
    parser = argparse.ArgumentParser(
        prog="firecrawl_amazon.py",
        description="Firecrawl Amazon product adapter (gated, free-tier fallback).",
    )
