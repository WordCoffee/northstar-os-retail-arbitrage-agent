...
One opt-in flag selects the provider:
    ...
    SCANNER_OFFER_ENRICHMENT = unset/empty | 0 | false | no | off | OFF
                             ...
                             | 1 | true | yes | on -> enabled, default provider
                              | AUTO | RAPIDAPI | DATAFORSEO | BRIGHTDATA
                              | CHOCODATA | UNWRANGLE -> enabled, that provider
                              | EASYPARSER -> DEPRECATED/DISABLED (degrades to cache-only)

No provider is ever selected from an unset variable: unset means local/
cache-only with zero outbound calls.

- BRIGHTDATA (default when enabled): enrich_product(asin or URL) — dataset
  results including any listing-reported FBA fee and monthly-sales signal
  the dataset provides.
- RAPIDAPI (explicit token only, never the bare-boolean default):
  get_rapidapi_offers(asin) — offer counts, observed FBA/FBM counts, Buy
  Box price and price range. The RapidAPI amazon-product-data4 host field
  paths are verified by a live probe before trusting non-null values;
  absent fields stay null.
- DATAFORSEO: dataforseo_adapter.get_dataforseo_offers(asin) — Merchant
  asin + sellers tasks. NOTE: DataForSEO does not return BSR, Buy Box
  price, or FBA/FBM flags; those fields are left null with data_gaps.
- EASYPARSER: DEPRECATED and DISABLED. Requesting it degrades to the
  offline placeholder (no network call).
...
- AUTO: try RapidAPI first; use Bright Data only when the RapidAPI
...
from rapidapi_client import get_rapidapi_offers
...
# ---------------------------------------------------------------------------
# Enrichment status vocabulary (versioned contract with the Scout UI).
#
# A provider-side rejection must NEVER be reported the same way as "no
# provider is configured right now": offline placeholders are genuinely
# cache-only / offline-mode, while a rejected or failed live fetch reports
# the rejection distinctly. Unknown values stay None, never 0.
# ---------------------------------------------------------------------------
ENRICHMENT_STATUS_NO_PROVIDER_CONFIGURED = "no_provider_configured"
ENRICHMENT_STATUS_PROVIDER_REJECTED = "provider_rejected"
ENRICHMENT_STATUS_PENDING = "pending"  # queued/in-flight fetch (reserved; not emitted by offline code paths)
ENRICHMENT_STATUS_OFFLINE_MODE = "offline_mode"

# Typed provider-fetch failure classes. A failure marker is any dict with
# ok=False + failure_class; it is never merged into a "no data found" result.
FETCH_FAILURE_CLASS_NOT_CONFIGURED = "not_configured"
FETCH_FAILURE_CLASS_AUTH_ERROR = "auth_error"
FETCH_FAILURE_CLASS_TRANSPORT_ERROR = "transport_error"
FETCH_FAILURE_CLASS_MALFORMED_RESPONSE = "malformed_response"


def _typed_fetch_failure(
    failure_class: str, detail: str, status_code: Optional[int] = None
) -> Dict:
    """Typed failure marker for provider fetches (never raises).

    Distinguished from a genuine empty result so callers can report a
    provider-side rejection distinctly from "no data found" and never
    swallow an auth/transport/malformed failure into an offline blank.
    """
    return {
        "ok": False,
...
def _is_fetch_failure(data: Dict) -> bool:
...
def _enrichment_mode() -> str:
    ...
    # EASYPARSER is DEPRECATED and DISABLED — requesting it falls through to OFF.
    if raw in ("AUTO", "RAPIDAPI", "DATAFORSEO", "BRIGHTDATA", "CHOCODATA", "UNWRANGLE"):
        return raw
    if env_flag("SCANNER_OFFER_ENRICHMENT"):
        # Explicit 1/true/yes/on opt-in: enable the default provider.
        # BRIGHTDATA is the documented default PRIMARY (never a bare
        # boolean silently remapping to a live credit-spending provider).
        return "BRIGHTDATA"
    return "OFF"
...
def _map_easyparser_offer(data: Dict, asin: Optional[str]) -> Dict:
    ...
    if not any_data:
        # A provider fetch that produced no usable ranking data is a
        # provider-side rejection, never "no provider configured".
        return _apply_provenance(shape, "easyparser",
                                 ENRICHMENT_STATUS_PROVIDER_REJECTED,
...
def _map_chocodata_offer(data: Dict, asin: Optional[str]) -> Dict:
    ...
    """
    shape = get_offer_data(asin or "")
    if _is_fetch_failure(data):
        # Typed provider rejection: propagate the class into data_gaps and
        # report a provider-side rejection, never an offline blank.
        shape["data_gaps"] = list(shape.get("data_gaps") or [])
        shape["data_gaps"].append(
            "chocodata %s: %s" % (data.get("failure_class"),
                                  data.get("failure_detail")))
        return _apply_provenance(shape, "chocodata",
                                 ENRICHMENT_STATUS_PROVIDER_REJECTED, _now_iso())
    if not data:
        return shape
...
def _fetch_chocodata_product(asin: str) -> Dict:
    ...
    Failures are classified and returned as a typed marker dict
    (``ok: False`` + ``failure_class``), never conflated with a genuine
    "no data found" result: not-configured (missing key), auth_error
    (401/403), transport_error (network/timeout/HTTP), and
    malformed_response (bad JSON or non-object) are distinct. Returns the
    raw JSON object on success.
    """
    if not CHOCODATA_API_KEY:
        return _typed_fetch_failure(
            FETCH_FAILURE_CLASS_NOT_CONFIGURED, "CHOCODATA_API_KEY not set")
    try:
        ...
        return _typed_fetch_failure(
            FETCH_FAILURE_CLASS_TRANSPORT_ERROR, "request failed: %s" % exc)
    if response.status_code in (401, 403):
        return _typed_fetch_failure(
            FETCH_FAILURE_CLASS_AUTH_ERROR, "HTTP %d" % response.status_code,
            response.status_code)
    if response.status_code != 200:
        return _typed_fetch_failure(
            FETCH_FAILURE_CLASS_TRANSPORT_ERROR, "HTTP %d" % response.status_code,
            response.status_code)
    try:
        data = response.json()
    except (json.JSONDecodeError, ValueError):
        return _typed_fetch_failure(
            FETCH_FAILURE_CLASS_MALFORMED_RESPONSE,
            "response body was not valid JSON")
    if not isinstance(data, dict):
        return _typed_fetch_failure(
            FETCH_FAILURE_CLASS_MALFORMED_RESPONSE,
            "response expected a JSON object")
    return data
...
def _fetch_unwrangle_amazon_detail(asin: str) -> Dict:
    ...
    (401/403), transport_error (network/timeout/HTTP), and
    malformed_response (bad JSON or non-object) are distinct. Returns the
    provider's ``detail`` object on success.
    """
    if not UNWRANGLE_API_KEY:
        return _typed_fetch_failure(
            FETCH_FAILURE_CLASS_NOT_CONFIGURED, "UNWRANGLE_API_KEY not set")
    try:
        response = requests.get(
            UNWRANGLE_URL,
            params={
    ...
    if not isinstance(detail, dict):
        return _typed_fetch_failure(
            FETCH_FAILURE_CLASS_MALFORMED_RESPONSE,
            "response detail expected a JSON object")
    return detail
...
def _map_unwrangle_offer(data: Dict, asin: Optional[str]) -> Dict:
    ...
    if _is_fetch_failure(data):
        shape["data_gaps"] = list(shape.get("data_gaps") or [])
        shape["data_gaps"].append(
            "unwrangle %s: %s" % (data.get("failure_class"),
                                  data.get("failure_detail")))
        return _apply_provenance(shape, "unwrangle",
                                 ENRICHMENT_STATUS_PROVIDER_REJECTED, _now_iso())
    if not data:
        return shape
...
def _offline_offer(identifier: Optional[str],
                   status: str = ENRICHMENT_STATUS_NO_PROVIDER_CONFIGURED) -> Dict:
...
def _offline_offer_from_candidate(
    ...
    enrichment_status "offline_mode": they are never presented as live,
    ...
    shape["enrichment_status"] = ENRICHMENT_STATUS_OFFLINE_MODE
...
def _offline_offer_merged(
    ...
    remains "offline" and enrichment_status stays offline-mode
    (no_provider_configured/offline_mode) with snapshot_* metadata attached
    so snapshot data is never presented as a live fetch.
    """
...
def _fetch_live_offer(identifier: Optional[str], asin: Optional[str], mode: str) -> Dict:
    ...
    EASYPARSER is deprecated/disabled and degrades to the offline placeholder.
    """
    if mode == "EASYPARSER":
        # Deprecated and disabled: never call the network; degrade safely.
        return _offline_offer(identifier, "easyparser_disabled")

    if mode == "RAPIDAPI":
        if not asin:
            return _offline_offer(identifier, "invalid_asin")
        return _map_easyparser_offer(get_rapidapi_offers(asin), asin)

    if mode == "DATAFORSEO":
        if not asin:
            return _offline_offer(identifier, "invalid_asin")
        return _map_easyparser_offer(
            dataforseo_adapter.get_dataforseo_offers(asin), asin
        )
    ...
    if mode == "AUTO":
        if not asin:
            return _offline_offer(identifier, "invalid_asin")
        rap = _map_easyparser_offer(get_rapidapi_offers(asin), asin)
        if rap.get("enrichment_status") == "complete":
            return rap
...
def get_scanner_offer(identifier: Optional[str]) -> Dict:
    ...
    EASYPARSER is a disabled/deprecated token and degrades to the offline
    placeholder (OFF); RAPIDAPI maps the RapidAPI normalized offer shape;
    DATAFORSEO maps the DataForSEO Merchant asin+sellers tasks; BRIGHTDATA
    maps the Bright Data normalized result; CHOCODATA maps the
    /amazon/product page; UNWRANGLE maps the amazon_detail page (1
    credit/ASIN for Amazon US); AUTO tries RapidAPI first and falls back to
    Bright Data only when the RapidAPI result is missing fields the ranking
    model needs (sellers, FBA sellers, Buy Box price, range) or the provider
    failed. Anything else returns the offline placeholder (zero network
    calls, nulls for unknown fields). Never raises: provider failures
    degrade to the placeholder/best-effort shape.
    ...
    if mode == "OFF":
        # No enrichment provider is configured for this scan: cache-only.
        return _offline_offer(identifier, ENRICHMENT_STATUS_NO_PROVIDER_CONFIGURED)
...
def map_seller_offer_contract(data: Optional[Dict], asin: Optional[str]) -> Dict:
    ...
    if not isinstance(data, dict) or data.get("source") not in (
        "easyparser", "rapidapi", "dataforseo"
    ):
        contract["offer_data_status"] = SELLER_STATUS_PROVIDER_ERROR
        contract["offer_data_note"] = (
            "Seller roster provider unavailable or disabled."
        )
        return contract

    gaps = [g for g in (data.get("data_gaps") or []) if isinstance(g, str)]
    contract["offer_data_source"] = data.get("source") or "easyparser"
