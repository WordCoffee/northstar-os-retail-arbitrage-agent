...
Normalization rules (shared with every other provider):
  ...
  - any failure records LAST_ERROR (typed: auth_error | http_error |
...
def _fetch(url: str) -> Optional[str]:
    ...
    non-200 or network failure records LAST_ERROR (typed: auth_error for
    401/403, http_error for other non-200 statuses, transport_error for
    request/connection failures) and returns None — a provider failure is
    NEVER reported the same way as "no data found". Raises ValueError only
    ...
    try:
        ...
        # Unexpected provider-layer failure: still typed, still surfaced,
        # never silently confused with an empty-but-fine result.
        LAST_ERROR = "transport_error: %s" % exc
        print(f"[Bright Data] transport_error: {exc}")
        return None
    ...
    if response.status_code != 200:
        LAST_ERROR = "http_error: HTTP %d" % response.status_code
