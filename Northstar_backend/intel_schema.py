...
# BSR (Best Sellers Rank) capture states. "verified" = provider returned a
# parsed rank+category; "missing_csv_reference" = benchmark CSV carried no BSR
# for this ASIN (no benchmark anchor, not a live-provider gap);
# "missing_live" = live provider returned no BSR for a non-excluded ASIN;
# "provider_not_supported" = the chosen provider does not supply BSR at all;
# "parse_error" = a raw BSR string could not be parsed to a rank;
# "identity_conflict" = BSR came back for a conflicting identity (wrong
# title/brand/pack) and was rejected; "unavailable" = no BSR signal at all.
BSR_STATUSES = (
    "verified",
    "missing_live",
    "missing_csv_reference",
...
def normalize_bsr(raw: Any, source: Any = None) -> Dict[str, Any]:
    """Parse a raw BSR string into a structured, null-first BSR fact.

    Never infers a rank from price/reviews/category. "Not listed on page"
    (or empty) yields status 'unavailable' with rank null — never 0. A rank
    is only an int when it is a valid positive number; the raw string is
    always preserved. The optional secondary ranked category is captured too.
    """
    result = {
        "bsr_raw": raw if raw is not None else None,
        "bsr_primary_rank": None,
        "bsr_primary_category": None,
        "bsr_secondary_rank": None,
        "bsr_secondary_category": None,
        "bsr_capture_status": "unavailable",
        "bsr_source": source if source is not None else None,
    ...
    if not text or "not listed" in text.lower():
        result["bsr_capture_status"] = "unavailable"
        return result
    pairs: List[tuple] = []
    ...
    if not pairs:
        result["bsr_capture_status"] = "parse_error"
        return result
    primary = pairs[0]
    secondary = pairs[1] if len(pairs) > 1 else (None, None)
    result["bsr_primary_rank"] = primary[0]
    result["bsr_primary_category"] = primary[1]
    result["bsr_secondary_rank"] = secondary[0]
    result["bsr_secondary_category"] = secondary[1]
    result["bsr_capture_status"] = "verified"
    return result


def bsr_field_completeness(bsr: Dict[str, Any]) -> Dict[str, str]:
    """Per-field capture status for a normalized BSR fact.

    Each field reports one of: captured | unavailable | provider_not_supported
    | parse_error | identity_conflict | not_requested.
    """
    if not isinstance(bsr, dict):
        return {"bsr": "unavailable"}
    status = bsr.get("bsr_capture_status", "unavailable")
    if status in ("missing_live", "missing_csv_reference", "provider_not_supported",
                  "parse_error", "identity_conflict"):
        return {
            "bsr_primary_rank": status,
            "bsr_primary_category": status,
            "bsr_secondary_rank": status,
            "bsr_secondary_category": status,
        }
    if status == "verified":
        return {
            "bsr_primary_rank": "captured" if bsr.get("bsr_primary_rank") is not None else "unavailable",
            "bsr_primary_category": "captured" if bsr.get("bsr_primary_category") is not None else "unavailable",
            "bsr_secondary_rank": "captured" if bsr.get("bsr_secondary_rank") is not None else "unavailable",
            "bsr_secondary_category": "captured" if bsr.get("bsr_secondary_category") is not None else "unavailable",
        }
    return {
        "bsr_primary_rank": "unavailable",
        "bsr_primary_category": "unavailable",
        "bsr_secondary_rank": "unavailable",
        "bsr_secondary_category": "unavailable",
    }
