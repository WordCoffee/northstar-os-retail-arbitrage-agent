...
# EASYPARSER is DEPRECATED/DISABLED and must resolve to OFF (cache-only),
# never to a live provider or to its own identity.
PROVIDER_TOKENS = ("AUTO", "BRIGHTDATA", "CHOCODATA", "UNWRANGLE")
DEPRECATED_DISABLED_TOKEN = "EASYPARSER"
...
class CacheOnlyScannerTests(unittest.TestCase):
    ...
    def test_boolean_values_enable_default_provider(self):
        for value in ENABLED_BOOLEAN_VALUES:
            with self.subTest(value=value), self._env(value):
                # Boolean opt-in enables the default provider (BRIGHTDATA),
                # never a bare truthy string silently selecting another
                # live credit-spending provider.
    ...
    def test_deprecated_provider_token_is_cache_only(self):
        # EASYPARSER is DEPRECATED/DISABLED: requesting it must resolve
        # to OFF (cache-only, zero outbound calls), not to itself, not to
        # a live fallback provider.
        with self._env(DEPRECATED_DISABLED_TOKEN):
