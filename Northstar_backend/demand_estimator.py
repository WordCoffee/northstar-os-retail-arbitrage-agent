...
# The fee engine and Amazon BSR department text use a different taxonomy
# than these demand curves (e.g. referral "Health & Personal Care" vs BSR
# "Health & Household", "Beauty" vs "Beauty & Personal Care", "Consumer
# Electronics" vs "Electronics"). Bridge the vocabularies so a single
# upstream category string resolves to the correct demand curve. No curve
# values change — only the alias mapping.
CATEGORY_ALIASES = {
...
def _canonical_category(category: Optional[str]) -> Optional[str]:
    """Map a fee/BSR category string onto a supported demand curve key."""
    if not isinstance(category, str) or not category.strip():
        return None
    folded = " ".join(category.lower().split())
    if folded in CATEGORY_CURVES:
        return folded
    return CATEGORY_ALIASES.get(folded)
...
def estimate_demand(
    ...
    bsr_category_clean = _canonical_category(bsr_category)
