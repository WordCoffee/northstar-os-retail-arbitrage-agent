"""scrape.do — Amazon product page adapter (free-tier fallback).

One request per ASIN via scrape.do GET API (raw HTML). Parses the same
seller data contract via the shared amazon_seller_extract parser so
records are byte-compatible with the Bright Data adapter.

Free tier (verified 2026-09-13): 1,000 requests/month, residential/mobile
proxies + JS rendering + CAPTCHA solving included.

Gating (fail-closed):
  SCRAPEDO_AMAZON_DETAIL_ENABLED=1 required for live calls. Default 0.

Discipline (mirrors firecrawl_costco.py / bright_data_costco.py):
  - one request per item via requests.get, ZERO retries
  - sequential pacing: SCRAPEDO_AMAZON_REQUEST_DELAY_SECONDS (default 2.0)
  ...
  - exact HTTP status via LAST_HTTP_STATUS, typed LAST_ERROR string on every
    ...
    the run-directory convention, keyed with the `scrapedo_` prefix.

Env vars:
  SCRAPEDO_AMAZON_DETAIL_ENABLED        (default "0"; "1" enables live calls)
  SCRAPE_DO_API_KEY                     (required for live; value never logged)
  SCRAPEDO_AMAZON_REQUEST_URL           (default https://api.scrape.do)
  SCRAPEDO_AMAZON_REQUEST_DELAY_SECONDS (default 2.0)
  SCRAPEDO_AMAZON_REQUEST_TIMEOUT_SECONDS (default 90)
"""
...
GATE_ENV = "SCRAPEDO_AMAZON_DETAIL_ENABLED"
DELAY_ENV = "SCRAPEDO_AMAZON_REQUEST_DELAY_SECONDS"
DEFAULT_DELAY_SECONDS = 2.0
KEY_ENV = "SCRAPE_DO_API_KEY"
REQUEST_URL_ENV = "SCRAPEDO_AMAZON_REQUEST_URL"
DEFAULT_REQUEST_URL = "https://api.scrape.do"
TIMEOUT_ENV = "SCRAPEDO_AMAZON_REQUEST_TIMEOUT_SECONDS"
...
def _gate_enabled() -> bool:
    """Live-call gate: SCRAPEDO_AMAZON_DETAIL_ENABLED must be '1'."""
...
def _fetch_item_page(url: str) -> Optional[str]:
    """One scrape.do request; returns raw HTML or None on hard failure.
    ...
    for the query parameter and never included in any persisted payload.
    """
    ...
    if not key:
        ...
        raise ValueError("%s not configured; cannot run live scrape.do calls." % KEY_ENV)

    params = {"token": key, "url": url, "render": "true"}
    headers = {"Accept": "text/html"}

    try:
        resp = requests.get(_request_url(), params=params, headers=headers, timeout=_timeout())
    ...
    if resp.status_code == 402:
        LAST_ERROR = "credits_exhausted: scrape.do free allowance exhausted"
        return None
    ...
    html = resp.text or ""
    if not html:
        LAST_ERROR = "malformed_response: empty body"
        return None
...
def _persist_evidence(run_dir: str, ts: str, asin: str, raw_record: Dict, norm_record: Dict) -> List[str]:
    ...
    raw_path = os.path.join(raw_dir, f"scrapedo_{asin}_{ts}.json")
    norm_path = os.path.join(norm_dir, f"items_{asin}_scrapedo_{ts}.json")
...
def refresh_product_details(
    ...
    """Fetch Amazon product pages via scrape.do.

    Gated (SCRAPEDO_AMAZON_DETAIL_ENABLED=1). One request per ASIN, zero
    ...
    if not _gate_enabled():
        msg = (
            f"[ScrapeDoAmazon] BLOCKED: {GATE_ENV} != '1'. "
    ...
    run_dir = run_dir or os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "data", "enrich", "scrapedo-amazon", "runs"
    )
    ...
    for idx, asin in enumerate(asins or []):
        ...
        if html is None:
            ...
            fail_rec = {
                "run_id": "scrapedo_amazon_%s" % ts,
                "attempt": "scrapedo_amazon_%s" % asin,
                "requested_at": requested_at,
                "provider": "SCRAPE_DO",
                "platform": "scrapedo_amazon",
        ...
        if seller_data["total_sellers"] is None and not seller_data["other_sellers_present"]:
            # No seller count and no other-sellers section — could be a blocked/empty page
        ...
        raw_record = {
            ...
            "platform": "scrapedo_amazon",
            "endpoint": _request_url(),
            "payload": {"url": url, "token": "<redacted>", "render": "true"},
            "auth_header": "token <redacted>",
...
def _cli(argv=None):
    parser = argparse.ArgumentParser(
        prog="scrapedo_amazon.py",
        description="scrape.do Amazon product adapter (gated, free-tier fallback).",
    )
