...
const provProducts = [
    ...
    P({ asin: 'B0TEST0005', name: 'Offline Item', offer_data_provider: 'offline', enrichment_status: 'offline', enriched_at: null }),
];
...
assert(els.proofBatchPanel.hidden === false, 'p17: proof batch panel unhidden when selection non-empty');
...
vm.runInContext('NS.renderProofBatch()', context);
assert(els.proofBatchPanel.hidden === true, 'p17: panel hides when selection empty');
...
assert(html.includes('Selection only'), 'p17: selection-only label present in static markup');
assert(html.includes('Live enrichment requires a separate guarded workflow and human confirmation'), 'p17: disabled live-enrichment line present, no run button');
