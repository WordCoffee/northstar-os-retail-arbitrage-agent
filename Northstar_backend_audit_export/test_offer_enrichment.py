...
def offline_shape(status="offline"):
    return {
        "amazon_price": 0,
        "buy_box_price": None,
        "lowest_price": None,
...
class EasyparserMappingTests(unittest.TestCase):
    def test_maps_offer_counts_buybox_and_range(self):
        with patch.object(offer_enrichment, "get_easyparser_offers", return_value=easyparser_payload()):
            with enrichment_mode("EASYPARSER"):
                result = offer_enrichment.get_scanner_offer("B000000001")
    ...
    def test_extracts_asin_from_url(self):
        mock_get = patch.object(offer_enrichment, "get_easyparser_offers", return_value=easyparser_payload()).start()
        try:
            with enrichment_mode("EASYPARSER"):
    ...
    def test_invalid_asin_falls_back_to_placeholder(self):
        mock_get = patch.object(offer_enrichment, "get_easyparser_offers").start()
        try:
            with enrichment_mode("EASYPARSER"):
    ...
    def test_provider_failure_returns_null_shape_without_raising(self):
        with patch.object(offer_enrichment, "get_easyparser_offers", return_value=failed_easyparser_payload()):
            with enrichment_mode("EASYPARSER"):
                result = offer_enrichment.get_scanner_offer("B000000001")
        ...
        self.assertEqual(result["offer_data_provider"], "easyparser")
        self.assertEqual(result["enrichment_status"], "failed")


class AutoModeTests(unittest.TestCase):
    def test_auto_uses_easyparser_when_complete(self):
    ...
    def test_auto_falls_back_to_brightdata_when_easyparser_incomplete(self):
        payload = dict(easyparser_payload())
        payload["offer_count"] = None
        payload["observed_fba_offer_count"] = None
        payload["observed_fbm_offer_count"] = None
        payload["buy_box_price"] = None
        with patch.object(offer_enrichment, "get_easyparser_offers", return_value=payload):
    ...
    def test_auto_brightdata_partial_still_wins_over_broken_easyparser(self):
    ...
    def test_auto_invalid_asin_goes_offline(self):
        with patch.object(offer_enrichment, "get_easyparser_offers") as mock_ep:
...
class EnrichmentCacheTests(unittest.TestCase):
    ...
    def test_fresh_cache_hit_skips_provider_call(self):
        ...
        mock_provider = patch.object(offer_enrichment, "get_easyparser_offers").start()
        try:
            with self._cache_enabled():
                with enrichment_mode("EASYPARSER"):
                    result = offer_enrichment.get_scanner_offer("B000000001")
    ...
    def test_stale_cache_entry_is_ignored_and_refreshed(self):
        ...
        with open(_TEST_CACHE_PATH, "w", encoding="utf-8") as f:
            json.dump(cache, f)
        mock_provider = patch.object(offer_enrichment, "get_easyparser_offers", return_value=easyparser_payload()).start()
        try:
            with self._cache_enabled():
                with enrichment_mode("EASYPARSER"):
                    result = offer_enrichment.get_scanner_offer("B000000001")
    ...
    def test_failed_fetch_is_never_cached(self):
        mock_provider = patch.object(offer_enrichment, "get_easyparser_offers", return_value=failed_easyparser_payload()).start()
        try:
            with self._cache_enabled():
                with enrichment_mode("EASYPARSER"):
                    result = offer_enrichment.get_scanner_offer("B000000001")
