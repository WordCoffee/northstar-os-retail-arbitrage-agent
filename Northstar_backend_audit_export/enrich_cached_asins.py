"""Controlled, resumable Easyparser batch enrichment for cached ASINs.

Three run modes:

    python enrich_cached_asins.py --mode status
    python enrich_cached_asins.py --mode dry-run [--limit N]
    python enrich_cached_asins.py --mode preflight --live --asin B0XXXXXXXXXX \
        --limit 1 --max-requests 1 --max-credits N
    python enrich_cached_asins.py --mode batch --live --limit N \
        --max-requests N --max-credits N [--retries N]

- status / dry-run: zero network calls. They read the candidate cache and
  the snapshot store and plan the request/credit budget.
- preflight: exactly ONE live Easyparser OFFER request for exactly one
  explicitly supplied ASIN, then an atomic per-ASIN snapshot save.
...
Budget: requests and credits are counted per request. Credit usage is
taken from the provider response (credits_used) when returned; otherwise
the documented ~5 credits per Easyparser OFFER request is used as the
estimate for planning and accounting.

Only Easyparser is ever called by this command. It never invokes Bright
Data, Scavio, Costco, Chocodata, Unwrangle, Amazon/SP-API, or any
fallback provider.
...
def _run_dry_run(args) -> int:
    ...
    print("credits projected:     ~%.0f (Easyparser ~%.0f/ASIN)" % (credits_est, ESTIMATED_CREDITS_PER_ASIN))
    print("stages:                offers (Easyparser OFFER); sales not requested")
    report = {
        "mode": MODE_DRY_RUN,
...
def _fetch_once(asin: str, caps: Dict[str, int], budget: Dict[str, float]) -> Dict:
    """One Easyparser OFFER request, budget-aware. Never raises."""
...
def _run_preflight(args) -> int:
    ...
    result = _fetch_once(asin, caps, budget)
    ...
    print("mode:                  preflight (exactly one Easyparser OFFER request)")
    print("asin:                  %s" % asin)
...
def _run_batch(args) -> int:
    ...
    print("mode:                  batch (live Easyparser OFFER)")
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Controlled Easyparser batch enrichment for cached ASINs "
    ...
    parser.add_argument("--live", action="store_true", help="allow live Easyparser requests")
