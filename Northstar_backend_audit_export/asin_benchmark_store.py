...
def parse_bsr(raw: Any) -> Dict[str, Any]:
    """First #N in Category fragment -> rank_number/category; raw kept.

    Category text in the source carries a scraping artifact (trailing
    "amazon") which is stripped before classification.
    """
    if raw is None:
        return {"raw": None, "rank_number": None, "category": None}
    text = str(raw).strip()
    if not text or "not listed" in text.lower():
        return {"raw": text, "rank_number": None, "category": None}
    rank_number = None
    category = None
    num_match = _BSR_NUM_RE.search(text)
    if num_match:
        try:
            rank_number = int(num_match.group(1).replace(",", ""))
        except ValueError:
            rank_number = None
    cat_match = _BSR_CAT_RE.search(text)
    if cat_match:
        category = _TRAILING_AMAZON_RE.sub("", cat_match.group(1)).strip()
        if not category:
            category = None
    return {"raw": text, "rank_number": rank_number, "category": category}
